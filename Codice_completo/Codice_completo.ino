...
boolean dist_misuration2(){//Questa funzione resistuisce il valore booleano true se la quantita di aqua è sopra una certa soglia
  digitalWrite(triggerPort2, LOW);
  digitalWrite(triggerPort2, HIGH);
  delayMicroseconds(10);
  digitalWrite(triggerPort2, LOW);//Invio del segnale di trigger al sensore
  long durata = pulseIn(echoPort2,HIGH);//Questa funzione restituisce quanto tempo passa dall'invio del segnale alla sua ricezione
  long spazio = 0.034 * durata/2; //calcolo dello spazio ottenuto  moltiplicando la velocità del suono in centimetri/microsecondi per il tempo che il segnale ha impiegato per compiere una certa distanza diviso 2
  if (spazio > 9){
   return HIGH;
  }
